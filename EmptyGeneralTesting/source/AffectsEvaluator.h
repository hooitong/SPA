...
private:
  ...
  vector<int> getAllLeft(const QNode* const node, const QueryResult &result) const;
  vector<int> getAllRight(const QNode* const node, const QueryResult &result) const;
  bool checkRight(const QNode* const node, const int tested) const;
  bool checkLeft(const QNode* const node, const int tested) const;

  bool findPathToNode(STMTLINE current, STMTLINE end, VARINDEX contextVar, vector<STMTLINE> path) const;
};