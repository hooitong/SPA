...
void TNode::getAllChildrenIncludeSubByTTypeAndValue(vector<TNode*> &children, TType type, string value) {

  for (int i = 0; i < this->getChildren().size(); i++) {
    TNode* child = this->getChildren()[i];
	if (child->getTType() == type && child->getValue() == value) children.push_back(child);
    child->getAllChildrenIncludeSubByTType(children, type);
  }
